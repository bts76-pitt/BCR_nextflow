...
#'Utility function for bulk data to standard Platypus format conversion, customized by Brent Schlegel for use with the latest MIXCR output.
...
custom_VDJ_bulk_to_vgm <- function(VDJ.bulk.out.directory.list,
                                   input.type,
  ...
  if (missing(clone.strategy)) {
    ...
    if (input.type == "MAF" |
        input.type == "")
      message("No clone.strategy provided. Data will be clonotyped by CDR3")
    if (input.type == "MIXCR")
      message("No clone.strategy provided. 10x clonotyping will be kept")
    if (input.type == "TRUST4" |
        input.type == "TRUST4.FULL")
      message("No clone.strategy provided. Sequence_id values will be used as clonotypes")
  }
  else {
    want.clonotyping = TRUE
  }
  ...
  if (length(group.id) != length(VDJ.bulk.out.directory.list) && !is.na(group.id)) {
    stop(
      "numeric group.id vector should be of the same length as
          the number of samples in VDJ.bulk.out.directory.list."
    )
  }
  
  if (length(batches) != length(VDJ.bulk.out.directory.list) &&
      !is.na(batches)) {
    stop(
      "numeric batches vector should be of the same length as
          the number of samples in VDJ.bulk.out.directory.list."
    )
  }
  ...
  if (input.type == 'MIXCR') {
    ...
    for (i in 1:length(VDJ.bulk.out.directory.list)) {
      VDJ.dataframe.list[[i]] <-
        utils::read.table(
          VDJ.bulk.out.directory.list[[i]],
          sep = "\t",
          encoding = "UTF-8",
          header = TRUE
        )
    ...
    i <-
      substr(data.df$allCHitsWithScore, 1, 3) %in% c("IGH", "") # subset the heavy chains
    j <-
      substr(data.df$allCHitsWithScore, 1, 3) %in% c("IGL", "IGK") # subset the light chains
    ...
    VDJ_vgene[i] <-
      gsub("\\[^,]*", "",  data.df$allVHitsWithScore[i]) %>% gsub("\\s*\\([^\\)]+\\)", "", .)
    VDJ_vgene[i] <- gsub(",", ";", VDJ_vgene[i])
    
    #17:
    VJ_vgene <- rep(NA, nrow(data.df))
    VJ_vgene[j] <-
      gsub("\\*[^,]*", "", data.df$allVHitsWithScore[j])
    ...
    VDJ_cgene[i] <-
      gsub("\\*[^,]*", "", data.df$allCHitsWithScore[i])
    VDJ_cgene[i] <- gsub(",", ";", VDJ_cgene[i])
    
    #22:
    VJ_cgene <- rep(NA, nrow(data.df))
    VJ_cgene[j] <-
      gsub("\\*[^,]*", "", data.df$allCHitsWithScore[j])
    ...
    if (all(names_n %in% colnames(data.df))) {
      VDJ_sequence_nt_trimmed[i] <- paste0(
        data.df$nSeqFR1[i],
        data.df$nSeqCDR1[i],
        data.df$nSeqFR2[i],
        data.df$nSeqCDR2[i],
    ...
    if (all(names_n %in% colnames(data.df))) {
      VJ_sequence_nt_trimmed[j] <- paste0(
        data.df$nSeqFR1[j],
        data.df$nSeqCDR1[j],
        data.df$nSeqFR2[j],
        data.df$nSeqCDR2[j],
    ...
    #34:
    clonotype_frequency <- data.df$readCount
    ...
    VDJ_nSeqCDR3[i] <-
      data.df$nSeqCDR3[i] # CDR3 is the minimum information we expect to always have
    ...
    if (want.clonotyping == TRUE & vgm.expanded == TRUE) {
      ...
      clonotype_frequency_ <-
        paste0("clonotype_frequency_", clone.strategy)
      
      vgm.per.cell$clonotype_id <- vgm.per.cell[[clonotype_id_]]
      vgm.per.cell$clonotype_id_10x <- vgm.per.cell[[clonotype_id_]]
      vgm.per.cell$clonotype_frequency <-
        vgm.per.cell[[clonotype_frequency_]]
      ...
      print(
        "Re-clonotyping can only be carried out if both want.clonotyping and vgm.expanded arguments are set to TRUE"
      )
    }
    ...
    for (i in 1:length(VDJ.bulk.out.directory.list)) {
      VDJ.dataframe.list[[i]] <-
        utils::read.delim(VDJ.bulk.out.directory.list[[i]])
      VDJ.dataframe.list[[i]]$sample_id <- paste0("s", i)
      VDJ.dataframe.list[[i]]$group_id <- group.id[i]
      VDJ.dataframe.list[[i]]$batches <- batches[i]
      data.df <- rbind(data.df, VDJ.dataframe.list[[i]])
    ...
    VDJ_cgene <-
      toupper(paste0(
        substr(data.df$Isotype, 2, 3),
        "H",
        substr(data.df$Isotype, 4, 4)
      ))
    ...
    vgm.clonotyped <-
      VDJ_clonotype(
        ...
        VDJ.VJ.1chain = F,
        hierarchical = 'none'
      ) # TRY ALSO hierarchical = 'single.chains':
    ...
    for (n in names(vgm.clonotyped)) {
      if (length(unique(vgm.clonotyped[[n]])) > 1 &
          !(n %in% c("clonotype_id", "sample_id", "barcode")))
    ...
    vgm.bulk.list[[1]] <- vgm.clonotyped %>%
      ...
      dplyr::summarise(dplyr::across(.cols = dplyr::all_of(columns_to_summarise) , most_frequent),
                       dplyr::across(),
                       .groups = "keep") %>%
      dplyr::distinct()
    ...
    vgm.bulk.list[[1]] <-
      vgm.bulk.list[[1]][order(as.numeric(gsub(
        ".*?([0-9]+).*", "\\1", vgm.bulk.list[[1]]$sample_id
      )), as.numeric(
        gsub(".*?([0-9]+).*", "\\1", vgm.bulk.list[[1]]$clonotype_id)
      )), ]
    ...
    i <-
      substr(data.df$c_call, 1, 3) %in% c("IGH", "TRB", "TRD", "") # subset the heavy chains
    j <-
      substr(data.df$c_call, 1, 3) %in% c("IGL", "IGK", "TRA", "TRG") # subset the light chains
    ...
    #48
    clonotype_id <-
      data.df$sequence_id   ### In TRUST4, sequence_id is not unique for each read. We use it as clonotype when no clonotyping strategy is specified as argument
    ...
    else if (integrate.MIXCR.output == TRUE &
             input.type == "TRUST4.FULL") {
...
VDJ_plot_SHM.custom <- function(VDJ.mixcr.matrix,
                                group.by,
                                quantile.label,
                                point.size,
                                mean.line.color,
  ...
  platypus.version <- "v3"
  if (missing(quantile.label))
    quantile.label <- 0.9
  if (missing(group.by))
    group.by <- "sample_id"
  if (missing(point.size))
    point.size <- 2
  if (missing(mean.line.color))
    mean.line.color <- "black"
  if (missing(stats.to.console))
    stats.to.console <- F
  
  VDJ.matrix <- VDJ.mixcr.matrix
  
  #get data
  if (!"VDJ_SHM" %in% names(VDJ.matrix) |
      !"VJ_SHM" %in% names(VDJ.matrix)) {
    stop(
      "VDJ_SHM or VJ_SHM column not found in the input dataframe. Please provide a output dataframe of the VDJ_call_MIXCR function"
    )
  }
  ...
  if ((group.by %in% names(VDJ.matrix)) == F) {
    stop(
      paste0(
        "Specified group.by column " ,
        group.by,
        " was not found in VDJ.matrix input dataframe. Please provide a valid column name"
      )
    )
  }
  
  to_plot <-
    VDJ.matrix[, c(group.by, "barcode", "VDJ_SHM", "VJ_SHM")]
  ...
  means <-
    to_plot_long %>% dplyr::group_by(name, group) %>% dplyr::summarise(m = mean(value))
  
  box_plot <-
    ggplot2::ggplot(to_plot_long, ggplot2::aes(color = group, y = value, x = group)) + ggplot2::geom_jitter(alpha = 0.4,
                                                                                                            width = 0.35,
                                                                                                            size = point.size) + ggplot2::geom_errorbar(
                                                                                                              inherit.aes = F,
                                                                                                              data = means,
                                                                                                              ggplot2::aes(ymax = m, ymin = m, x = group),
                                                                                                              color = mean.line.color,
                                                                                                              size = 1.6,
                                                                                                              width = 0.85
                                                                                                            ) + ggplot2::theme_bw() + cowplot::theme_cowplot() + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) + ggplot2::ylab("SHM") + ggplot2::xlab("") + ggplot2::ggtitle(label = paste0("SHM per ", group.by)) + ggplot2::theme(strip.background = ggplot2::element_rect(color = "white", fill = "white"))
  ...
  vdjmean <-
    SHM_iso_VDJ %>% dplyr::group_by(group) %>% dplyr::summarize(m = mean(value))
  vjmean <-
    SHM_iso_VJ %>% dplyr::group_by(group) %>% dplyr::summarize(m = mean(value))
  
  #VDJ
  if (stats.to.console)
    message("VDJ chain SHM data summary")
  if (stats.to.console)
    message(as.data.frame(
      dplyr::group_by(SHM_iso_VDJ, group) %>%
        dplyr::summarise(
          count = dplyr::n(),
  ...
  if (stats.to.console)
    message("\n kruskal.test()")
  if (stats.to.console)
    message(stats::kruskal.test(value ~ group, data = SHM_iso_VDJ))
  if (stats.to.console)
    message("\n pairwise.wilcox.test(p.adjust.method = 'BH')")
  if (stats.to.console)
    suppressWarnings(message(
      stats::pairwise.wilcox.test(SHM_iso_VDJ$value, SHM_iso_VDJ$group, p.adjust.method = "BH")
    ))
  ...
  if (stats.to.console)
    message("------------")
  if (stats.to.console)
    message("VJ chain SHM data summary")
  if (stats.to.console)
    message(as.data.frame(
      dplyr::group_by(SHM_iso_VJ, group) %>%
        dplyr::summarise(
          count = dplyr::n(),
          mean = mean(value, na.rm = TRUE),
          sd = stats::sd(value, na.rm = TRUE),
  ...
  if (stats.to.console)
    message("\n kruskal.test()")
  if (stats.to.console)
    message(stats::kruskal.test(value ~ group, data = SHM_iso_VJ))
  if (stats.to.console)
    message("\n pairwise.wilcox.test(p.adjust.method = 'BH')")
  if (stats.to.console)
    suppressWarnings(message(
      stats::pairwise.wilcox.test(SHM_iso_VJ$value, SHM_iso_VJ$group, p.adjust.method = "BH")
    ))
...
#Function to calculate V Gene usage (modified from Platypus):
VGene_usage <-
  function(VDJ.matrix,
           Fraction.HC,
           HC.gene.number,
    ...
    for (i in 1:length(unique(VDJ.matrix$sample_id))) {
      clonotype.list[[i]] <-
        subset(VDJ.matrix, sample_id == unique(VDJ.matrix$sample_id)[i])
      #get unique clones
      clonotype.list[[i]] <-
        clonotype.list[[i]][duplicated(clonotype.list[[i]]$clonotype_id) == F, ]
    }
    names(clonotype.list) <- unique(VDJ.matrix$sample_id)
    message(paste0("Sample order: ", paste0(
      unique(VDJ.matrix$sample_id), collapse = " ; "
    )))
    ...
    for (i in 1:length(clonotype.list)) {
      #Calculate the V gene usage frequencies for each sample on the clonal level
      HC_Vgene_usage[[i]] <-
        as.data.frame(table(clonotype.list[[i]]$VDJ_vgene))
      colnames(HC_Vgene_usage[[i]]) <- c("Vgene", "Frequency")
      for (j in 1:nrow(HC_Vgene_usage[[i]])) {
        HC_Vgene_usage[[i]]$Percentage[j] <-
          HC_Vgene_usage[[i]]$Frequency[j] / sum(HC_Vgene_usage[[i]]$Frequency) *
          100
      }
      ...
      HC_Vgene_usage_fraction[[i]] <-
        HC_Vgene_usage[[i]][which(HC_Vgene_usage[[i]]$Percentage > Fraction.HC), ]
    }
    ...
    for (i in 1:length(clonotype.list)) {
      HC_Vgene_usage[[i]] <-
        HC_Vgene_usage[[i]][HC_Vgene_usage[[i]]$Vgene %in% HC_Vgene_names, ]
    }
    ...
    HC_Vgene_usage_all_unique <-
      data.frame("Vgene" = rep("", length(unique_Vgene)),
                 "Frequency" = rep(0, length(unique_Vgene)))
    HC_Vgene_usage_all_unique$Vgene <- unique_Vgene
    
    for (i in 1:length(unique_Vgene)) {
      HC_Vgene_usage_all_unique[i, "Percentage"] <-
        mean(HC_Vgene_usage_all[which(HC_Vgene_usage_all$Vgene == HC_Vgene_usage_all_unique$Vgene[i]), ]$Percentage)
    }
    ...
    for (i in 1:length(clonotype.list)) {
      HC_Vgene_usage_top[[i]] <-
        data.frame("Vgene" = rep("", length(top_Vgenes)),
                   "Frequency" = rep(0, length(top_Vgenes)))
      ...
      for (j in 1:nrow(HC_Vgene_usage_top[[i]])) {
        if (HC_Vgene_usage_top[[i]]$Vgene[j] %in% HC_Vgene_usage[[i]]$Vgene == TRUE) {
          HC_Vgene_usage_top[[i]]$Frequency[j] <-
            HC_Vgene_usage[[i]][which(HC_Vgene_usage[[i]]$Vgene == paste0(HC_Vgene_usage_top[[i]]$Vgene[j])), ]$Frequency
        }
      }
      for (j in 1:nrow(HC_Vgene_usage_top[[i]])) {
        HC_Vgene_usage_top[[i]]$Percentage[j] <-
          HC_Vgene_usage_top[[i]]$Frequency[j] / sum(HC_Vgene_usage_top[[i]]$Frequency) *
          100
      }
    }
    ...
    heatmap_df <- do.call("rbind", HC_Vgene_usage_top)
    heatmap_df <- heatmap_df[!is.na(heatmap_df$Vgene),]
    heatmap_df <- heatmap_df[heatmap_df$Vgene != "",]
    return(heatmap_df)
  }
